...
import { createSoftSkill, deleteSoftSkillById, findSoftSkillById, getAllSoftSkill } from '../operations/DB/softSkill.operation';
import { SoftSkill } from '../types/';

export const getAllSoftSkillCtrl = async (req: Request, res: Response) => {
    try {
        const foundSoftSkills = await getAllSoftSkill();
        if (!foundSoftSkills) {
            res.status(404).json({ message: 'Soft Skill not found' });
            return;
        }
        res.status(200).json({ softSkills: foundSoftSkills });
    } catch (error) {
